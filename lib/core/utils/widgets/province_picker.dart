...
Future<String?> showProvincePicker(BuildContext context, {String? selected}) {
...
class ProvinceSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Material لا Container: الصندوق الملوَّن يحجب تموّج لمس ListTile —
    // ونبّه عليه Flutter 3.47 بتأكيد صريح. والتموّج كان مفقوداً فعلاً.
    return Material(
...
class ProvinceField extends FormField<String> {
  ProvinceField({
    ...
    bool required = true,
         ...
         initialValue: value,
         validator: required
             ? (v) => (v == null || v.isEmpty) ? 'الرجاء اختيار المحافظة' : null
             : null,
         builder: (field) {
               ...
               InkWell(
                 ...
                 child: Container(
                   ...
                   decoration: BoxDecoration(
                     ...
                     borderRadius: BorderRadius.circular(12.r),
                     border: Border.all(
                       color: field.hasError ? MyColors.error : MyColors.border,
                     ),
                   ),
                   child: Row(
                     children: [
                       Icon(
                         Icons.location_on_outlined,
                         size: 20.sp,
                         color: hasValue ? MyColors.primary : MyColors.textHint,
                       ),
                       SizedBox(width: 10.w),
                       Expanded(
                         child: Text(
